...
function addUser() {
    ...
    $.ajax({
        ...
        error: function(err) {
            ...
            let message = 'Không thể thêm user. Vui lòng thử lại!';
            
            if (err.responseJSON && err.responseJSON.errors) {
                const allErrors = Object.values(err.responseJSON.errors); // array các mảng lỗi
                if (allErrors.length > 0 && allErrors[0].length > 0) {
                    message = allErrors[0][0]; // lấy message đầu tiên
                }
            }
            Swal.fire({
                icon: 'error',
...
$(document).ready(function() {
            ...
            <div class="modal-body">
                <form id="userForm">
                    <div class="form-group">
                        ...
                        <input id="name" type="text" name="name" class="form-control" placeholder="Nhập tên" required>
                    </div>
                    <div class="form-group">
                        <label>Số liên hệ:</label>
                        <input id="phone" type="text" name="phone" class="form-control" placeholder="Nhập số liên hệ" required>
                    </div>
                    <div class="form-group">
                        <label>Email:</label>
    ...
    $(document).on('click', '#kt_search_4', function(e) {
        ...
        const searchId = $('#input_id').val().trim();
        const searchName = $('#input_name').val().trim();
        const searchEmail = $('#input_email').val().trim();
